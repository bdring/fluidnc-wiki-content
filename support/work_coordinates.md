...
Newcomers to CNC often assume there is only one XYZ coordinate system.  While some kinds of machines like 3D printers do have only one coordinate system, CNC machines and full G-code usually have multiple coordinate systems.  Some user interface programs try to hide that, showing you only one coordinate system, but that simplified view will eventually lead to confusion. 
