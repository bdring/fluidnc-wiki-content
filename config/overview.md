...
FluidNC's config file format has a lot of sections and fields to support a wide range of machines, so it's easy for both people and AI assistants (ChatGPT, Claude, etc.) to get details wrong. The [FluidNC repository](https://github.com/bdring/FluidNC) includes a set of tools to help generate and check config files, aimed at both humans and AI assistants.

- **[`FluidNC/docs/config_items.yaml`](https://github.com/bdring/FluidNC/blob/main/FluidNC/docs/config_items.yaml)** — The complete field reference: every section, every field, with its type, valid range, default, and a description, **generated directly from annotations in the FluidNC source code** (so it cannot drift out of date). This is the authoritative list of what exists. The [Config Items Index](/config/config_items_index) is a browsable version of the same data.
- **[`tools/fluidnc-config-spec.md`](https://github.com/bdring/FluidNC/blob/main/tools/fluidnc-config-spec.md)** — The companion prose reference: the file **grammar** (indentation, comments, quoting, pin-string syntax), whole-document and cross-field rules that no per-field list can express, worked examples, and the common mistakes AI assistants make. It does **not** repeat the per-field tables — read it together with `config_items.yaml`, not instead of it.
- **[`tools/validate_fluidnc_config.py`](https://github.com/bdring/FluidNC/blob/main/tools/validate_fluidnc_config.py)** — A command-line tool that checks a `config.yaml` and reports any problems (it builds its validation schema from `config_items.yaml` at run time). Run it with `python3 validate_fluidnc_config.py your_config.yaml` (it offers to install its own small set of dependencies automatically). Add `--permissive` to allow harmless mixed-case section/field names (FluidNC itself matches these case-insensitively) while still catching real mistakes.
- **[`tools/fluidnc_config_mcp_server.py`](https://github.com/bdring/FluidNC/blob/main/tools/fluidnc_config_mcp_server.py)** — An [MCP](https://modelcontextprotocol.io/) server exposing the same validation as a tool call. AI assistants that support MCP (Claude Desktop, Claude Code, and others) can use this to check a config they're generating *before* handing it to you.
- **[`tools/fluidnc_validate_core.py`](https://github.com/bdring/FluidNC/blob/main/tools/fluidnc_validate_core.py)** and **[`tools/config_schema_adapter.py`](https://github.com/bdring/FluidNC/blob/main/tools/config_schema_adapter.py)** — Shared validation logic used by the CLI tool and the MCP server above; most people won't need to use these directly.
...
> 1. Read **both** [`FluidNC/docs/config_items.yaml`](https://github.com/bdring/FluidNC/blob/main/FluidNC/docs/config_items.yaml) (the exhaustive per-field reference — every section, field, type, range, default) **and** [`tools/fluidnc-config-spec.md`](https://github.com/bdring/FluidNC/blob/main/tools/fluidnc-config-spec.md) (the grammar, cross-field rules, and worked examples) in the `bdring/FluidNC` GitHub repo. The spec document does not duplicate the field tables and is not sufficient on its own.
> 2. Validate what you produced — via the MCP server (`fluidnc_config_mcp_server.py`) if you have tool access, by running `validate_fluidnc_config.py` if you have code execution, or by manually checking against the two documents above if you have neither. **The validator works fine on a bare fragment** (e.g. just a `Relay:` block with nothing else) — you do not need a complete file to check your work.
> 3. Fix anything the validator flags before showing your answer to the user.
>
> **A specific, confirmed example of what goes wrong if you skip this:** three different AI assistants (Claude, GitHub Copilot, and Gemini), independently, have each invented a wrapper key around spindle content — `spindle:`, `Spindle:`, or `spindles:` — that does not exist in FluidNC's format. A spindle type name (`PWM`, `Relay`, `ModbusVFD`, etc.) is a direct top-level key, exactly like `axes:` or `control:`, never nested inside anything else. This is a strong, convergent mistake because it's how virtually every other config format (and FluidNC's own maintainers, in hindsight) would design it — so don't assume your own instinct here is correct just because it feels natural. The validator catches this immediately.
